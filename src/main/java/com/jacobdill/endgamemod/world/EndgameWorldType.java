...
public class EndgameWorldType extends WorldType{
	...
	public ChunkGenerator<?> createChunkGenerator(World world){
		...
		EndBiomeProviderSettings provider = new EndBiomeProviderSettings();
		
		settings.setDefaultBlock(Blocks.END_STONE.getDefaultState());
		settings.setDefaultFluid(Blocks.AIR.getDefaultState());
		
		return ChunkGeneratorType.FLOATING_ISLANDS.create(world, new ExampleBiomeProvider(provider), settings);
	...
	public class ExampleBiomeProvider extends EndBiomeProvider{
		...
		public Set<BlockState> getSurfaceBlocks() {
			if (this.topBlocksCache.isEmpty()) {
				for(Biome biome : this.field_205009_d) {
					//if(biome instanceof ExampleBiome) this.topBlocksCache.add(EndgameBlocks.END_SOIL.getDefaultState());
					//else 
